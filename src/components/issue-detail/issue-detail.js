...
export default class IssueDetail extends Component {
    ...
    cardBody = () => {
        const { address, avtomatNumber, createdAt, email, phoneNumber, issue } = this.state.issueDetail;
        const { comment } = this.state;
        ...
        return (
            <div className="card-body">
                ...
                <p className="card-text text-muted">
                    ...
                    <a target='_blank' rel='noopener noreferrer' className='text-info'
                       href={`/order?avtomat_number=${avtomatNumber}&date=${createdAt.split(' ')[0]}`}>
                        {avtomatNumber}&nbsp;<i className="fas fa-external-link-alt fa-xs"></i>
                    </a>
                </p>
                <p className="card-text text-muted">
                ...
                <p className="card-text text-muted">
                    <i className="card-icon fas fa-phone"></i>&nbsp;{phoneNumber}
                </p>
                <p>{ issue }</p>
